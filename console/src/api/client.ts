 ...
 *
 * That is the rule the console is subordinate to: every number it shows comes from
 * a command the CLI also runs. A component reaching for the network is how a
 * second data layer starts.
 *
 * No runtime schema check. The TypeScript shapes are not an independent
 * declaration any more -- `contract.py` declares them, FastAPI exports them, and
 * `types.gen.ts` is generated on every build -- so a type that disagrees with the
 * server cannot be committed. A parser here would mean hand-writing the schemas
 * again.
 *
 ...
 *
 * The server not being there YET, which is the ordinary state for the first
 * seconds of `just console-dev`: Vite is ready in ~3s and the Python server is
 * not, so every query fired in that window fails permanently. A panel on an
 * interval heals itself; `configs` (`staleTime: Infinity`, no interval) stays
 * broken for the life of the tab.
 *
 * Two shapes, and neither is the server declining to answer. 502 is our own "the
 * body was not JSON", which in dev is Vite handing back the SPA for a refused
 * connection. A bare `TypeError` is `fetch` failing to connect at all, which is
 * what production looks like when `serve` is down.
 *
 * Everything else is not retried: a 422 is a refusal the server will repeat, and a
 * 503 means Azure did not answer, which needs `az login` rather than patience.
 */
