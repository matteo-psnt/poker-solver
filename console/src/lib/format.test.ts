...
describe("taskLabel", () => {
  it("strips the queue-time suffix, leaving what the task does", () => {
    expect(taskLabel("score-production-1095-150M-seed7-090456-18475")).toBe(
  ...
  it("keeps three same-checkpoint evals telling themselves apart", () => {
    const ids = ["seed7-090456-18475", "seed13-090501-1849", "seed29-090506-24091"].map((tail) =>
      taskLabel(`score-production-1095-150M-${tail}`),
    );
    expect(new Set(ids).size).toBe(3);
  });

  it("reduces a pre-label id to its run, which is all it ever carried", () => {
    expect(taskLabel("run-production-025433-1095-090456-18475")).toBe("run-production-025433-1095");
  });
  ...
  it("strips only the trailing queue suffix when the label itself ends in digits", () => {
    ...
    expect(taskLabel("score-x-250000-3-090456-18475")).toBe("score-x-250000-3");
  });
});
