...
class TestProgressHeartbeat:
    ...
    def test_a_training_task_publishes_how_far_it_has_got(self, paths, monkeypatch):
        ...
        runner.publish_progress(paths, self._plan(), {"iteration": 250})
    ...
    def test_a_kind_with_nothing_to_say_writes_nothing(self, paths, monkeypatch):
        ...
        monkeypatch.setenv("AZ_BATCH_TASK_ID", "t-1")
        runner.publish_progress(paths, self._plan(op=TaskName.PRECOMPUTE), {"iteration": 1})
    ...
    def test_it_is_refreshed_in_place_rather_than_accumulated(self, paths, monkeypatch):
        ...
        for iteration in (100, 200, 300):
            runner.publish_progress(paths, self._plan(), {"iteration": iteration})
    ...
    def test_an_unwritable_share_never_kills_the_task(self, paths, monkeypatch):
        ...
        monkeypatch.setenv("AZ_BATCH_TASK_ID", "t-1")
        monkeypatch.setattr(
            runner.task_log, "write_progress_record", _boom := lambda *a, **k: 1 / 0
        )
        runner.publish_progress(paths, self._plan(), {"iteration": 1})
    ...
    def test_a_finished_task_stops_showing_a_bar(self, paths, monkeypatch):
        ...
        task stuck at 62%."""
        monkeypatch.setenv("AZ_BATCH_TASK_ID", "t-1")
        runner.publish_progress(paths, self._plan(), {"iteration": 620})
...
class TestTheWatcherReportsPromptly:
    def test_a_task_shorter_than_the_interval_still_reports(self, paths, log, monkeypatch):
        ...
        monkeypatch.setattr(runner, "_node_state", lambda *a: {"iteration": 400})

        watcher = runner.LadderWatcher(paths, log, interval=9999, plan=plan)
        watcher.start()
        watcher.stop()
...
class TestProgressReadsThisTasksRun:
    ...
    def test_an_earlier_runs_manifest_is_not_mistaken_for_this_one(self, paths, monkeypatch):
        ...
        assert runner._training_state(paths, plan) == {"iteration": 80_000}
    ...
    def test_work_done_is_measured_against_this_tasks_own_baseline(self, paths, monkeypatch):
        ...
        runner.note_baseline(paths, plan)
        self._manifest(paths, "run-a", 150_000_000)

        assert runner._units_done(paths) == 10_000_000
...
def test_every_kind_the_submit_path_accepts_has_something_to_run_it():
    ...
    tests/shared/test_tasks.py, so a kind cannot exist on one side only. This is
    the third place: a kind with no executor validates, builds an argv and earns
    a label, then raises KeyError on the node -- after a snapshot upload, a pool
    spin-up and every Batch retry. Exactly the "found three of four" failure the
    whole abstraction exists to remove.
    """
    missing = sorted(set(tasks.TaskKind.KINDS) - {str(name) for name in runner.HANDLERS})
