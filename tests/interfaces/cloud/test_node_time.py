...
def _task(start_min: float, end_min: float | None) -> dict[str, object]:
...
class TestNodeTime:
    ...
    def test_an_unfinished_task_is_credited_up_to_now(self):
        ...
        result = node_time.summarise([_task(300, None)], now=NOW)
        assert result["task_hours"] == pytest.approx(1.0)
...
class TestTimeline:
    def test_it_tracks_concurrency_up_and_down(self):
        spans = node_time.intervals([_task(0, 60), _task(30, 90)], now=NOW)
    ...
    def test_simultaneous_events_collapse_to_one_point(self):
        ...
        spans = node_time.intervals([_task(0, 60) for _ in range(3)], now=NOW)
