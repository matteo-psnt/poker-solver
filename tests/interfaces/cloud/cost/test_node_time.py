...
def _task(
    ...
    return task_history.TaskRow(
        task_id="t",
        attempt=1,
        cause=cause,
        cause_source="node",
        started_at=(base + timedelta(minutes=start_min)).isoformat(),
        ended_at=None if end_min is None else (base + timedelta(minutes=end_min)).isoformat(),
    )


def _bare(**over: Any) -> task_history.TaskRow:
    """A row missing the fields the others have -- the degenerate cases below."""
    return task_history.TaskRow(
        task_id="t", attempt=1, cause="unknown", cause_source="none", **over
    )


def _live(start_min: float) -> task_history.TaskRow:
...
class TestNodeTime:
    ...
    def test_a_task_with_no_start_is_skipped_not_fatal(self):
        result = node_time.summarise([_bare(ended_at=NOW.isoformat()), _task(0, 60)], now=NOW)
    ...
    def test_a_task_with_no_start_is_not_counted_as_unended(self):
        ...
        result = node_time.summarise([_bare(ended_at=NOW.isoformat())], now=NOW)
        assert result.unended == 0

    def test_a_malformed_timestamp_is_skipped(self):
        result = node_time.summarise([_bare(started_at="not-a-date"), _task(0, 60)], now=NOW)
        assert result.tasks == 1
