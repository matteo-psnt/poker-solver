...
from src.pipeline.services.experiments import ArmResult
...
class ReportPayload(services.ExperimentReport):
    """Every arm of an experiment, attributed against its control."""

    op: Literal["report"] = "report"


def run(args: argparse.Namespace) -> ReportPayload:
    ...
    return ReportPayload(**out.model_dump())


def render(payload: ReportPayload) -> None:
...
__all__ = ["COMMAND", "ArmResult", "add_arguments", "render", "run"]
